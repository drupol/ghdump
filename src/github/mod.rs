    ...
    thread_order
        ...
        .filter_map(|thread_id| {
            ...
            let path = comments_by_id
                ...
                .or_else(|| grouped_comments.iter().find_map(|comment| comment.path.clone()));
...
fn to_timeline_entry(event: &RestTimelineEvent) -> TimelineEntry {
    ...
    TimelineEntry {
        ...
        created_at: event
            ...
            .or_else(|| event.author.as_ref().and_then(|author| author.date.as_ref().cloned()))
            .or_else(|| {
                event
                    .committer
                    .as_ref()
...
mod tests {
    ...
    use super::{
        ...
        to_commit_summary, to_label, to_milestone, to_reactions, to_review,
        to_review_comment, to_timeline_entry,
    };
    ...
    fn committed_timeline_event_uses_sha_message_and_author_date() {
        ...
        assert_eq!(commit_author.email.as_deref(), Some("aliaksandr@example.org"));
    }
